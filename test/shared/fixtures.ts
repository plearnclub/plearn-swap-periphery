...
export async function v3Fixture([wallet, burnWallet, teamWallet]: Wallet[], provider: Web3Provider): Promise<V3Fixture> {
  // deploy tokens
  const tokenA = await deployContract(wallet, ERC20, [expandTo18Decimals(10000)])
  const tokenB = await deployContract(wallet, ERC20, [expandTo18Decimals(10000)])
  const WETH = await deployContract(wallet, WETH9)
  ...
  const feeHandler = await deployContract(wallet, PlearnFeeHandler, [tokenA.address, router02.address, wallet.address, burnWallet.address, [tokenA.address, tokenB.address]], overrides)
  const feeManager = await deployContract(wallet, PlearnFeeManager, [tokenA.address, feeHandler.address, router02.address, teamWallet.address, 6000, 50, 0], overrides)
